...
class GameState:
    def __init__(self):
        ...
        self.mirror_move_functions = {'p': self.get_mirror_pawn_moves, 'c': self.get_mirror_cannon_moves,
                                      'r': self.get_mirror_chariot_moves,
                                      'n': self.get_mirror_horse_moves, 'b': self.get_mirror_elephant_moves,
                                      'a': self.get_mirror_advisor_moves,
                                      'k': self.get_mirror_king_moves, 'u': self.get_mirror_undefined_piece_moves}
        ...
        self.mirror_pos_board = copy.deepcopy(self.pos_board)
        for row in self.pos_board:
            row.reverse()
        self.mirror_pos_board.reverse()
        self.mirror_red_pieces = copy.deepcopy(self.red_pieces)
        self.mirror_black_pieces = copy.deepcopy(self.black_pieces)
    ...
    def make_mirror_move(self, move):
        ...
        if move.piece_moved[1] == 'u':
            self.mirror_board[move.end_row][move.end_col] = self.mirror_red_pieces[-1] if self.red_to_move else self.mirror_black_pieces[-1]
        else:
    ...
    def get_mirror_advisor_moves(self, r, c, moves):
        ...
        for d in directions:
            ...
            end_col = c + d[1]
            if self.red_to_move:
                if self.mirror_board[r][c] == 'ru':
                    if 0 <= end_row <= 2 and 3 <= end_col <= 5:
                        end_piece = self.mirror_board[end_row][end_col]
                        if end_piece[0] != ally_color:
                            moves.append(Move((r, c), (end_row, end_col), self.mirror_board))
                if self.mirror_board[r][c] == 'ra':
                    if 0 <= end_row <= 9 and 0 <= end_col <= 8:
                        end_piece = self.mirror_board[end_row][end_col]
                        if end_piece[0] != ally_color:
                            moves.append(Move((r, c), (end_row, end_col), self.mirror_board))
            else:
                if self.mirror_board[r][c] == 'bu':
                    if 7 <= end_row <= 9 and 3 <= end_col <= 5:
                        end_piece = self.mirror_board[end_row][end_col]
                        if end_piece[0] != ally_color:
                            moves.append(Move((r, c), (end_row, end_col), self.mirror_board))
                if self.mirror_board[r][c] == 'ba':
                    if 0 <= end_row <= 9 and 0 <= end_col <= 8:
                        end_piece = self.mirror_board[end_row][end_col]
                        if end_piece[0] != ally_color:
                            moves.append(Move((r, c), (end_row, end_col), self.mirror_board))
    ...
    def get_mirror_undefined_piece_moves(self, r, c, moves):
        if len(self.mirror_pos_board[r][c]) == 2:
            self.mirror_move_functions[self.mirror_pos_board[r][c][1]](r, c, moves)
